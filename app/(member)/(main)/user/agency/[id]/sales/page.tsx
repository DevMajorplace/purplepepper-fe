...
export default function AgencySales() {
  ...
  const COLUMNS = [
    ...
    columnHelper.accessor("cash", {
      id: "cash",
      header: () => (
        <TableTh text="보유 캐시" onSort={() => handleSort("company")} />
      ),
    ...
    columnHelper.accessor("savings", {
      id: "savings",
      header: () => (
        <TableTh text="이번 달 적립금액" onSort={() => handleSort("savings")} />
      ),
      cell: (info) => `${info.getValue().toLocaleString()} 원`,
    }),
  ...
  return (
    <div className="w-full h-full scroll-bar flex flex-col gap-5">
      <div className="flex flex-col ">
        <SalesTable className={"w-[600px] [&_th]:w-[200px]"} data={salesData} />
        <ClientTable
          className={"w-[800px] [&_th]:w-[200px]"}
          data={clientData}
        />
      </div>
