...
const FAQS = [
  ...
  {
    question: "내가 올린 광고는 어디서 확인이 가능할까요?",
    answer: (
      <>
  ...
  {
    question: "카드 결제는 불가한가요?",
    answer: <>카드 결제는 아직 지원하고 있지 않는 점 양해 부탁드립니다.</>,
  },
  {
    question: "문의는 어떻게 접수하나요?",
    answer: (
      <>
...
export default function Faq() {
  return (
    <Card className="min-h-[86vh] pt-2 pb-10">
      {FAQS.map((item, index) => (
        <Accordion key={index} answer={item.answer} question={item.question} />
      ))}
    </Card>
  );
}
